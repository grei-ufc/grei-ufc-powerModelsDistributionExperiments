...
function objective_mc_min_cost(pm::AbstractUnbalancedPowerModel; report::Bool=true)
    gen_cost = Dict()

    for (n, nw_ref) in nws(pm)
        for (i,gen) in nw_ref[:gen]
            pg = sum(var(pm, n, :pg, i))
            injection = log(1 + exp(100*pg) ) / 100
            load = log(1 + exp(-100*pg) ) / 100
            gen_cost[(n,i)] = - gen["cost"][1]*injection + gen["cost"][2]*load
    ...
    for (n, nw_ref) in nws(pm)
        for (i,storage) in nw_ref[:storage]
            ...
            discharge = log(1 + exp(-100*ps) )/100
            charge = log(1 + exp(100*ps))/100
            storage_cost[(n,i)] = -storage["cost"][1]*discharge + storage["cost"][2]*charge
        end
    end
